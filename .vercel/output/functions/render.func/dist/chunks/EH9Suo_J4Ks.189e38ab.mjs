import './astro.33314ce0.mjs';

const id = "EH9Suo_J4Ks.mdx";
const collection = "videos";
const slug = "eh9suo_j4ks";
const body = "  \nTest Driven Development is a fantastic way to build code that you know is well tested and will work. I know I should code this way, but why don't I? Let's find out what Test Driven Development is, and see if you should give it a try as we apply it to basic JavaScript and TypeScript functions, custom React hooks and React components.\n";
const data = {
  title: "Test Driven Development: The best way to code that I almost never use",
  publishedAt: new Date(1635775157e3),
  tags: ["test driven development", "test driven development typescript", "test driven development javascript", "tdd", "tdd javascript", "tdd typescript", "tdd react testing library", "react testing library", "testing react hooks", "testing react hooks with react-testing-library", "what is test driven development", "test driven development tutorial", "what is tdd", "jack herrington", "test react componenets", "TDD'ing React hooks", "Testing without Wallaby", "TDD'ing React components"],
  privacyStatus: "public",
  short: false,
  thumbnails: {
    default: {
      url: "https://i.ytimg.com/vi/EH9Suo_J4Ks/default.jpg",
      width: 120,
      height: 90
    },
    medium: {
      url: "https://i.ytimg.com/vi/EH9Suo_J4Ks/mqdefault.jpg",
      width: 320,
      height: 180
    },
    high: {
      url: "https://i.ytimg.com/vi/EH9Suo_J4Ks/hqdefault.jpg",
      width: 480,
      height: 360
    },
    standard: {
      url: "https://i.ytimg.com/vi/EH9Suo_J4Ks/sddefault.jpg",
      width: 640,
      height: 480
    },
    maxres: {
      url: "https://i.ytimg.com/vi/EH9Suo_J4Ks/maxresdefault.jpg",
      width: 1280,
      height: 720
    }
  }
};
const _internal = {
  filePath: "/Users/gmercer/Desktop/astro/astro-bc-videos/src/content/videos/EH9Suo_J4Ks.mdx",
  rawData: `
title: "Test Driven Development: The best way to code that I almost never use"
publishedAt: 2021-11-01T13:59:17Z
thumbnails:
  default:
    url: https://i.ytimg.com/vi/EH9Suo_J4Ks/default.jpg
    width: 120
    height: 90
  medium:
    url: https://i.ytimg.com/vi/EH9Suo_J4Ks/mqdefault.jpg
    width: 320
    height: 180
  high:
    url: https://i.ytimg.com/vi/EH9Suo_J4Ks/hqdefault.jpg
    width: 480
    height: 360
  standard:
    url: https://i.ytimg.com/vi/EH9Suo_J4Ks/sddefault.jpg
    width: 640
    height: 480
  maxres:
    url: https://i.ytimg.com/vi/EH9Suo_J4Ks/maxresdefault.jpg
    width: 1280
    height: 720
tags:
  - test driven development
  - test driven development typescript
  - test driven development javascript
  - tdd
  - tdd javascript
  - tdd typescript
  - tdd react testing library
  - react testing library
  - testing react hooks
  - testing react hooks with react-testing-library
  - what is test driven development
  - test driven development tutorial
  - what is tdd
  - jack herrington
  - test react componenets
  - TDD'ing React hooks
  - Testing without Wallaby
  - TDD'ing React components
privacyStatus: public
short: false`
};

export { _internal, body, collection, data, id, slug };
