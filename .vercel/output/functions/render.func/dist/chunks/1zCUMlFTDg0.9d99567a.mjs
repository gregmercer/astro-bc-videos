import './astro.33314ce0.mjs';

const id = "1zCUMlFTDg0.mdx";
const collection = "videos";
const slug = "1zcumlftdg0";
const body = "  \nLet's use Module Federation to make Micro-Frontends easy!\n";
const data = {
  title: "Live! - Micro-FEs Simplified",
  publishedAt: new Date(1609059879e3),
  privacyStatus: "public",
  short: false,
  thumbnails: {
    default: {
      url: "https://i.ytimg.com/vi/1zCUMlFTDg0/default.jpg",
      width: 120,
      height: 90
    },
    medium: {
      url: "https://i.ytimg.com/vi/1zCUMlFTDg0/mqdefault.jpg",
      width: 320,
      height: 180
    },
    high: {
      url: "https://i.ytimg.com/vi/1zCUMlFTDg0/hqdefault.jpg",
      width: 480,
      height: 360
    },
    standard: {
      url: "https://i.ytimg.com/vi/1zCUMlFTDg0/sddefault.jpg",
      width: 640,
      height: 480
    },
    maxres: {
      url: "https://i.ytimg.com/vi/1zCUMlFTDg0/maxresdefault.jpg",
      width: 1280,
      height: 720
    }
  }
};
const _internal = {
  filePath: "/Users/gmercer/Desktop/astro/astro-bc-videos/src/content/videos/1zCUMlFTDg0.mdx",
  rawData: "\ntitle: Live! - Micro-FEs Simplified\npublishedAt: 2020-12-27T09:04:39Z\nthumbnails:\n  default:\n    url: https://i.ytimg.com/vi/1zCUMlFTDg0/default.jpg\n    width: 120\n    height: 90\n  medium:\n    url: https://i.ytimg.com/vi/1zCUMlFTDg0/mqdefault.jpg\n    width: 320\n    height: 180\n  high:\n    url: https://i.ytimg.com/vi/1zCUMlFTDg0/hqdefault.jpg\n    width: 480\n    height: 360\n  standard:\n    url: https://i.ytimg.com/vi/1zCUMlFTDg0/sddefault.jpg\n    width: 640\n    height: 480\n  maxres:\n    url: https://i.ytimg.com/vi/1zCUMlFTDg0/maxresdefault.jpg\n    width: 1280\n    height: 720\nprivacyStatus: public\nshort: false"
};

export { _internal, body, collection, data, id, slug };
